...
if (isset($_GET["dellog"]) && isset($_GET['qid'])&& isset($_GET['lid'])) {
   ...
   $stmt->execute();
   echo "<meta http-equiv='refresh' content='0; URL=index.php?id={$_GET['id']}&name={$_GET['name']}&lid={$_GET['lid']}'>";
   exit;
}
...
if (isset($_GET["name"])) {
   ...
   $table[4] = "Datum";
   $table[5] = "Löschen";
   $table[6] = "<small>Punkte o.G.</small>";
   $id = $_GET["id"];
} else {
   ...
   $table[4] = "";
   $table[5] = "";
   $table[6] = "<small>Gesamtpunkte o.G.</small>";
   $id = null;
}
?>
...
<html lang="de">
   ...
   <body>
     ...
     <div class="container mt-2">
       <?php echo((isset($_GET["name"])) ? "<p><a href='index.php'>zurück zur Übersicht</a></p>" : null);?>
     </div>
     ...
     <script>
       ...
       function loadResults(id) {
         ...
         var sum_full = 0;
         $.ajax({
            url: 'get_results.php?id=' + id, // Pfad zu deinem PHP-Skript, das die Ergebnisse lädt
            type: 'GET'
            , dataType: 'json'
            , success: function(data) {
              ...
              data.forEach(function(result) {
                ...
                if(result.id == 0) {
                  ...
                  sum_full += Number(result.total_score_full);
 ...
 <td>${result.total_score_full}</td>
 <td>${result.passage}</td>
 <td>${result.date}</td>
 <td data-exclude='true'><a href='index.php?id=${searchParams.get("id")}&name=${searchParams.get("name")}&qid=${result.rowid}&delrow=true'>Eintrag löschen</a></td>
 </tr>  `);
   ...
   <td></td>
   <td>${((log.quiz == 2) ? '' : parseFloat(log.points).toFixed(0))}</td>
   <td colspan='2'>${log.name}</td>
   <td data-exclude='true'>${((log.quiz == 1) ? '' : `<a href='index.php?id=${searchParams.get("id")}&name=${searchParams.get("name")}&qid=${result.rowid}&lid=${log.id}&points=${log.points}&dellog=true'>Eintrag löschen</a>`)}</td>
   </tr> `);
                  ...
                  if(headline != result.group_name) {
                     headline = result.group_name;
                     resultsBody.append(`
   <tr>
   <td colspan='6'>${result.group_name}</td>
   </tr>
   `);
                  }
                  resultsBody.append(`
   <tr>
   <td><a href='index.php?id=${result.id}&name=${result.name}'>${result.name}</a></td>
   <td>${result.total_score}</td>
   <td colspan='4'>${result.total_score_full}</td>
   </tr>
   `);
                }
   ...
   <tr>
   <td></td>
   <td>${parseFloat(sum).toFixed(0)}</td>
   <td>${parseFloat(sum_full).toFixed(0)}</td>
   <td colspan='3'></td>
   </tr>
   `);
              }
